...
class VideoUploader:
    ...
    def upload(self, video_path: Path, caption: str) -> bool:
        ...
        try:
            if self._perform_upload(video_path, caption, session_id):
    ...
    def _perform_upload(self, video_path: Path, caption: str, session_id: str) -> bool:
        """Execute the TikTok upload using the external uploader package."""

        try:
            from tiktok_uploader.auth import AuthBackend  # type: ignore[import]
            from tiktok_uploader.upload import upload_video  # type: ignore[import]
        except ImportError as exc:  # pragma: no cover - optional dependency
            logger.error(
                "tiktok-uploader package not installed. Add it to requirements to enable uploads.",
            )
            raise RuntimeError("TikTok uploader dependency missing") from exc

        auth = AuthBackend(session_id=session_id)
        logger.info("Uploading %s to TikTok via tiktok-uploader.", video_path)
        ...
        if not success:
            logger.error("TikTok uploader returned unsuccessful response: %s", response)
        return success
